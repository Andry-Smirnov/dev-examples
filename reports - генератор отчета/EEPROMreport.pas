{$CODEPAGE UTF8}
{
  @abstract(Модуль отчета SCADA EISA)
...
type
  ...
  TEEPROMReportForm = class(TForm)
    EEPROMReport: TfrxReport;
    ValuesUserDataSet: TfrxUserDataSet;
    procedure FormDestroy(Sender: TObject);
    procedure ValuesUserDataSetCheckEOF(Sender: TObject; var Eof: Boolean);
    procedure ValuesUserDataSetFirst(Sender: TObject);
    procedure ValuesUserDataSetNext(Sender: TObject);
    procedure ValuesUserDataSetGetValue(const VarName: string; var Value: Variant);
  private
    // Все настройки
    FGroups: TGroups;
    FGroup: Integer;
...
var
  EEPROMReportForm: TEEPROMReportForm;
